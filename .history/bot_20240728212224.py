...
async def get(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if file == "/send":
        ...
        reply = "Finding and Sending a requested file to you. Hold on..."
        await update.message.reply_text(reply)
        path = os.getcwd() + '/' + file
        if os.path.exists(path):
            await context.bot.send_document(chat_id=update.message.chat_id, document=open(path, 'rb'), timeout=100)
        else:
...
async def ls(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if file == "/show":
        ...
        path = os.getcwd() + '/' + file
        if os.path.exists(path):
            await update.message.reply_text(os.listdir(path))
        else:
